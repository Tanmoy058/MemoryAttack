...
{
    uint64_t buf_size = 1024*1024*1024; //declaration of the hugepage
    int fd = open("/mnt/hugepages/nebula1", O_CREAT|O_RDWR, 0755); //allocating file descriptor for the huge page
    if (fd<0)
    {
    ...
    buf = mmap(0, buf_size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0); //mapping the buffer
    if (buf == MAP_FAILED)
    {
        printf("map error!\n");
